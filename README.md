...
Each generated project includes a CLI tool named `openstudyindia`. Run commands from the `backend/` directory:

```bash
cd backend

# Server commands
uv run openstudyindia server run --reload     # Start dev server
uv run openstudyindia server routes           # Show all routes
# Database commands
uv run openstudyindia db migrate -m "message" # Create migration
uv run openstudyindia db upgrade              # Apply migrations
uv run openstudyindia db downgrade            # Rollback migration
# User commands
uv run openstudyindia user create-admin       # Create admin user
uv run openstudyindia user create             # Create regular user
uv run openstudyindia user list               # List all users
```
...
# Server
uv run openstudyindia server run --reload
uv run openstudyindia server routes

# Database (Alembic wrapper)
uv run openstudyindia db init
uv run openstudyindia db migrate -m "Add users"
uv run openstudyindia db upgrade

# Users
uv run openstudyindia user create-admin       # Create admin (interactive)
uv run openstudyindia user create             # Create user (interactive)
uv run openstudyindia user list               # List all users
```
...
```bash
uv run openstudyindia cmd seed --count 100
uv run openstudyindia cmd seed --dry-run
```
...
| Start dev server | `uv run --directory backend openstudyindia server run --reload` |
| Start prod server | `uv run --directory backend openstudyindia server run --host 0.0.0.0 --port 8000` |
| Show routes | `uv run --directory backend openstudyindia server routes` |
...
| Task | Command |
|------|---------|
| Create migration | `uv run --directory backend openstudyindia db migrate -m "message"` |
| Apply migrations | `uv run --directory backend openstudyindia db upgrade` |
| Rollback migration | `uv run --directory backend openstudyindia db downgrade` |
| Show current | `uv run --directory backend openstudyindia db current` |
| Show history | `uv run --directory backend openstudyindia db history` |

### Users

| Task | Command |
|------|---------|
| Create admin | `uv run --directory backend openstudyindia user create-admin` |
| Create user | `uv run --directory backend openstudyindia user create` |
| List users | `uv run --directory backend openstudyindia user list` |
