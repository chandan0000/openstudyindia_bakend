...
db-migrate:
	...
	uv run --directory backend openstudyindia db migrate -m "$$msg"

db-upgrade:
	uv run --directory backend openstudyindia db upgrade

db-downgrade:
	uv run --directory backend openstudyindia db downgrade

db-current:
	uv run --directory backend openstudyindia db current

db-history:
	uv run --directory backend openstudyindia db history

# === Server ===
run:
	uv run --directory backend openstudyindia server run --reload

run-prod:
	uv run --directory backend openstudyindia server run --host 0.0.0.0 --port 8000

routes:
	uv run --directory backend openstudyindia server routes

# === Users ===
create-admin:
	@echo "Creating admin user..."
	uv run --directory backend openstudyindia user create-admin

user-create:
	uv run --directory backend openstudyindia user create

user-list:
	uv run --directory backend openstudyindia user list
...
docker-db:
	...
	@echo "   Connection: postgresql://postgres:postgres@localhost:5432/openstudyindia"
...
help:
	@echo ""
	@echo "openstudyindia - Available Commands"
	@echo "======================================"
	@echo ""
	@echo "Setup:"
